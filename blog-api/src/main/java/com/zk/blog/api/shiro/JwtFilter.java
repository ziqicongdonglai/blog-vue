...
public class JwtFilter extends AuthenticatingFilter {
    ...
    @Override
    protected AuthenticationToken createToken(ServletRequest
                                                      servletRequest, ServletResponse servletResponse) throws Exception {
        HttpServletRequest request = (HttpServletRequest) servletRequest;
        String jwt = request.getHeader("Authorization");
        // 新的Spring中StringUtils的isEmpty⽅法已弃⽤，改为hasLength(),注意取反
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest,
        ...
        if (!StringUtils.hasLength(jwt)) {
            ...
            if (claim == null ||
                    jwtUtil.isTokenExpired(claim.getExpiration())) {
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse
        ...
        // 跨域时会⾸先发送⼀个OPTIONS请求，这⾥我们给OPTIONS请求直接返回正常状态
        if
        (httpServletRequest.getMethod().equals(RequestMethod.OPTIONS.name())) {
